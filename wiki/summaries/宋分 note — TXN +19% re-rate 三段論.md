---
title: 宋分 note — TXN +19% re-rate 三段論
aliases: [宋分 TXN re-rate, TXN +19%, 產業透鏡 #5]
type: summary
created: 2026-06-09
as_of: 2026-06-09
check_after: 2027-01-15
updated: 2026-06-09
sources:
  - raw/美股送分題-note-TXN-19漲-re-rate-2026-04-24.md
tags: [宋分, note, TXN, Re-rate, 類比晶片, 三段估值]
thesis_dependency: AI-capex
confidence: medium
---

# 宋分 note — TXN +19% 是利多反應還是 re-rate 開始？

[[宋分（美股送分題）]] 2026-04-24 substack note。**核心命題**：**新高 PE ≠ re-rate 結束**——TXN 從「估值壓縮階段」回到「敘事建立階段」。

## 三段估值 framework

| 階段 | 特徵 |
|---|---|
| 階段 1（敘事建立） | 財報驚喜出現、分析師上調、**PE 快速擴張** |
| 階段 2（基本面追上） | 強財報持續、股價盤整等獲利成長追上估值 |
| 階段 3（成長減速） | **估值壓縮** |

## TXN 的位置移動

```
階段 3（壓縮中）→ 階段 1（敘事重啟）
```

關鍵數據：
- 資料中心營收 **YoY +60% / QoQ +90%**
- 2026 PE 36x、2027 PE 32x
- 歷史平均 ~25x
- **AI 已變成「可見的成長來源」**

## 關鍵 distinction

> 預期已 price in、但 **earnings 驗證還沒完成**。

**敘事與基本面的時間差 = alpha 來源**。

## 實戰啟示

- 看到 PE 新高就退場 → 可能錯過 re-rate 全段
- 等 earnings 驗證來證明 PE 是真的 → 已經晚了
- **找「敘事剛建立、基本面還沒完全追上」的窗口**

## 與 wiki 既有 thesis 的對接

- [[宋分 ＃13 — 類比晶片結構性重估]]：TXN 是該 thesis 的核心標的
- [[Re-rate 捕捉法]]：補入「三段估值 framework」
- [[預期差]]：預期 price in、earnings 未驗證 = 標準預期差結構
- [[Texas Instruments TXN]] entity Step 3 需補入：「2026-04 +19% 移位（階段 3 → 階段 1）+ AI / 資料中心 60%/90% 成長 + 公平 PE 上限 35-38x」

## 相關連結

- [[宋分（美股送分題）]]
- [[宋分 ＃13 — 類比晶片結構性重估]]
- [[Re-rate 捕捉法]]
- [[預期差]]
- [[Texas Instruments TXN]]
