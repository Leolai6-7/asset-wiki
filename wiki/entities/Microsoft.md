---
title: Microsoft
aliases: [Microsoft, MSFT, Azure, Copilot]
type: entity
created: 2026-06-04
as_of: 2026-06-04
check_after: 2027-01-15
updated: 2026-06-09
sources:
  - raw/2026-04-06_微軟每月收你30美元用Copilot——但條款寫著僅供娛樂.md
  - raw/2026-04-06_OpenAI拿了1220億——但這筆錢不是你以為的那種錢.md
  - raw/2026-05-02_FOMOSOC-KP40-五大巨頭Q1財報AI變現分歧.md
  - raw/2026-05-09_FOMOSOC-KP41-AI利潤奇點CPU復興FDE.md
tags: [標的, M7, CSP, AI, OpenAI, Copilot, Azure, GitHub Copilot 使用量計費, AI 年化營收, 變現能見度中段, hyperscaler]
thesis_dependency: AI-capex
confidence: medium
---

# Microsoft

M7 之一，AI 時代最複雜的曝險點。**持股 OpenAI 27% + 最大 Azure CSP** = 雙重曝險於 AI 模型層風險。

## AI 業務四層

| 層 | 業務 |
|---|---|
| 模型層 | 持股 OpenAI 27% |
| 平台層 | Azure（最大 OpenAI 算力供應商） |
| 應用層 | Copilot（$30/月 SaaS） |
| 開發者層 | GitHub Copilot |

## 「Copilot 條款僅供娛樂」事件（2026-04-06）

→ 完整見 [[AI 訂閱制 unit economics]]

- Copilot 條款明文「**僅供娛樂**」+ 免責
- 揭穿：**$30/月不夠 cover 推論成本**
- NPS **-19.8**、滲透率卡在 **3.3%**
- = AI 訂閱制 unit economics 不 work 的證據

## 「四次重新定價」歷史（[[宋分 ＃10 — AI 半導體基礎建設化]]）

| Cycle | 時期 | 主軸 |
|---|---|---|
| 1 | PC 時代 | Windows / Office 壟斷 |
| 2 | 成熟期 | 「沒成長的現金牛」估值壓縮 |
| 3 | SaaS / Cloud（Nadella） | 估值擴張 |
| 4 | **AI 時代** | OpenAI 合作關係 |

## 跟既有 wiki 概念連結

- [[循環投資（CSP-Model 互鎖）]]：Microsoft ↔ OpenAI ↔ Azure 是教科書範例
- [[AI 訂閱制 unit economics]]：Copilot 是反例
- [[FCF 拐點]]：Azure CapEx 是核心追蹤點
- [[CapEx 見頂辯論]]：Microsoft Azure 是 CSP 永動機核心

## 雙重曝險風險

- OpenAI 任何危機 → 27% 持股價值 + Azure 訂單兩端皆受傷
- Copilot 推論成本不下降 → SaaS 毛利結構崩塌
- 對 [[Anthropic]] 沒投資 → AI 對齊路線完全押在 OpenAI

## ⭐ Q1 2026 財報 — AI 變現能見度中段 hyperscaler（2026-05-02 KP40）

→ 完整 framework 見 [[AI 變現能見度分歧（證明給我看階段）]]

### 變現執行驗證

| 指標 | 數字 |
|---|---|
| **AI 業務年化營收** | **$370 億**（YoY +123%）|
| **Copilot 付費座位** | **2,000 萬+**（環比 +33%）|
| **財富 500 強 AI 代理部署率** | **90%** |
| **OpenAI 關係** | 從獨家轉向**非獨家合作** |

### GitHub Copilot 6 月轉用量計費（戰略 pivot）

- **2026-06 起 GitHub Copilot 轉向「基於使用量計費」**
- 從「席位 $30/月」→「席位 + 用量混合定價」
- → 對接 [[AI 利潤奇點（Token 經濟學拐點）]]：用量計費 = 利潤跟產出綁定
- → 對接 [[控制點轉移（投資版）]]：商業模式「人頭 → 工作量」

### 市場反應溫和 driver

- AI 服務獨立收入不夠亮眼
- Copilot 付費數待驗證
- 從 OpenAI 獨家轉向非獨家 = 釋放更獨立的變現路徑

## 相關連結

- [[OpenAI]]
- [[Anthropic]]
- [[循環投資（CSP-Model 互鎖）]]
- [[AI 訂閱制 unit economics]]
- [[CapEx 見頂辯論]]
- [[FCF 拐點]]
- [[AI 變現能見度分歧（證明給我看階段）]]（KP40 framework）
- [[AI 利潤奇點（Token 經濟學拐點）]]（KP41 framework）
- [[Forward Deployed Engineer 戰略（FDE）]]（KP41 同源）
- [[AI infra CapEx 三階段論]]
- [[控制點轉移（投資版）]]
- [[NVDA]]、[[Google]]、[[Meta]]、[[Apple]]
- [[FOMO SOC]]（KP@FOMOSoc KOL 來源、待建）
- [[宋分（美股送分題）]]
