...
## 2026-06-09（L3 — 宋分 substack 後 9 URL fetch + 比對 + 補建 9 篇 summary）

### 觸發
9 個宋分 URL（後段 9、含 1 篇 substack note）的 fetch + 比對 + 補建。

### 編號衝突處理（重要）

⚠️ L1/L2 已用過 #24 / #25 / 備忘錄 #7 / #8 / #9 編號，L3 為避免衝突一律往後挪：

| 我 fetch 的 URL | 原文發布日 | URL slug | 落地編號 | 主題 |
|---|---|---|---|---|
| URL 19 | 2026-05-05 | amdcpu-ai | **#29** | AMD CPU AI 戰場與過度定價邊界 |
| URL 20 | 2026-05-09 | ac6 | **#30** | 連續上修 EPS 與市場三階段 |
| URL 21 | 2026-05-11 | 10ai-robotaxi | **備忘錄 #11** | 油價+AI+Robotaxi 改寫贏家 |
| URL 22 | 2026-05-18 | 11-ai | **備忘錄 #12** | AI 獲利擴散期 + 體驗經濟稀缺 |
| URL 23 | 2026-05-23 | 597 | **#33** | 安全邊際被誤解 |
| URL 24 | 2026-05-25 | 1220ai-agent-alpha | **備忘錄 #13** | Agentic Internet 入口革命 + UCP 控制權 |
| URL 25 | 2026-05-29 | 390-nvda | **#31** | 分析師底層系統 #3（資產負債表 NVDA $390 校準） |
| URL 26 | 2026-06-06 | e94 | **#32** | 左側 vs 右側交易與 AI 行情 |
| URL 27 | 2026-06-08 | note c-272461220 | **備忘錄 #14** | AI 成功速度不夠快 + 期待風險階段 |
...
6. **[[宋分備忘錄 ＃11 — 油價+AI+Robotaxi 改寫贏家]]**（2026-05-11、月度宏觀）⭐
   ...
   - AI 成購物中介（UCP 預告）
   - **車廠 SaaS 化**（Ford 10 億 / GM 30 億軟體營收）
   - **歐洲重卡 TaaS**（Volvo / Daimler / Traton 每台車終身 recurring revenue）
   - Robotaxi 玩家清單（Tesla / Waymo / Uber / Ford / GM / Volvo / Daimler / Traton）
...
7. **[[宋分備忘錄 ＃12 — AI 獲利擴散期 + 體驗經濟稀缺]]**（2026-05-18、月度宏觀）
   ...
   - **體驗經濟反直覺稀缺溢價**（迪士尼 / Live Nation / F1）
   - 「AI 不會摧毀稀缺、會放大稀缺」
...
8. **[[宋分備忘錄 ＃13 — Agentic Internet 入口革命 + UCP 控制權]]**（2026-05-25、月度宏觀）⭐
   ...
   - **Meta 隱藏 call option**（廣告公司 PE → AI 商業入口 PE）
   - 資訊優勢變成**問題優勢**（框架完整性 + 產業理解 + 市場嗅覺）
   - 流動性轉向實體 → 有現金流公司勝出
   - 對應 [[控制點轉移（投資版）]] 新 chokepoint
   - confidence: high
...
9. **[[宋分備忘錄 ＃14 — AI 成功速度不夠快 + 期待風險階段]]**（2026-06-08、月度宏觀、來源為 substack note c-272461220）⭐
   ...
   - **「期待風險」階段**（beat by 1% 不夠、要 beat by 5%+、guidance 持平不夠要 raise）
   - 「**最便宜模型**」航空業比喻（如經濟艙養活整個產業）
   - 第三階段 thesis 開場（AI 創造的價值最終留在誰的財報裡）
   - confidence: high

### 三個重點主題核心觀點

#### 1. Robotaxi（備忘錄 #11）
- 車廠商業模式轉變：產品 → 服務（recurring revenue）
- 受惠玩家分四層：北美 robotaxi (Tesla / Waymo) + 歐洲重卡 TaaS (Volvo / Daimler / Traton) + 美國車廠 SaaS 轉型 (Ford / GM) + 平台層 (Uber)
- **跨庫對接**：跟 llm-wiki 對照可深化（asset 看 recurring revenue 估值、llm 看 AI 技術 / 數據飛輪）
- Apple Cybercab 未在本期、屬同主題下一個 anchor

#### 2. AI Agent（備忘錄 #13）
- Google UCP = 新 chokepoint
- Agent = 新 iPhone 時刻
- Meta call option（被低估的 AI 商業入口）
- 對應現有 concept [[控制點轉移（投資版）]] 強化「Agent 入口控制」段
...
無重複既有 wiki 內容。9 個 URL 全為新主題。

### 連動更新

- 更新 [[wiki/index.md]]：加 4 條備忘錄 + 5 條教學系列
- 更新 [[wiki/log.md]]：本紀錄

### 文件清單（commit by batch）

- batch 1（commit 0d4bdc5）：#29 + #30 + 備忘錄 #11（raw + summary 共 6 檔）
- batch 2（commit fd416b3）：#33 + 備忘錄 #12 + 備忘錄 #13（raw + summary 共 6 檔）
- batch 3（本次 commit）：#31 + #32 + 備忘錄 #14 + 5 個 rename + index/log 更新
...
Leo 提供 9 個宋分 substack URL，要求 fetch + 跟既有 wiki/summaries/ 比對、補建未收錄。

#### URL 對照表（9 個）

| # | URL slug | 文章主題 | 日期 | 狀態 |
|---|---|---|---|---|
| 1 | db3 | 強勢股在下跌中發現（#18） | 2026-04-04 | 重複 |
| 2 | 5csp-fed | 備忘錄 #5 複利測試 | 2026-04-06 | 重複 |
| 3 | 6d8 | 能源結構性剛需（#20） | 2026-04-07 | 重複 |
| 4 | b24 | 反彈該不該上車（#21） | 2026-04-09 | 重複 |
| 5 | 6ai | 備忘錄 #6 市場世界觀切換 | 2026-04-13 | 重複 |
| 6 | 2epsre-rate | 分析師底層系統 #2 營業槓桿（#23） | 2026-04-14 | 重複 |
| 7 | ffe | 漲回來時三個錯（#24） | 2026-04-18 | **新建** |
| 8 | 6f4 | 機構獲利了結節奏（#25） | 2026-04-19 | **新建** |
| 9 | 7csp-aitoken-re-rate | 備忘錄 #7 CSP Token 定價權 | 2026-04-20 | **新建** |
...
- 備忘錄 #5 vs #7 形成「**CSP thesis 完整 V 型反轉**」：#1 在 2026-03 是恐慌期 CSP 被當資本黑洞、#7 在 2026-04 是反向定價落地。同一作者短短一個月之間從「檢驗」轉「擁護」、anchor 了 [[宋分（美股送分題）]] 訊息蛻變快的 KOL 特徵（[[KOL source reliability（KOL 來源可靠度評估）]] 12/15 命中率高、蛻變快）。
- 備忘錄 #7 跟 FOMO SOC KP41 高盛「Token 經濟學拐點」**同陣營反向證明**：高盛走「售價降 40% vs 成本降 60-70% 剪刀差」、宋分走「用量爆 10x + 定價權」→ 兩路徑收斂到「CSP margin 改善」、雙重 anchor 增加 thesis confidence。
- 全部 9 個 URL 沒有付費牆（之前付費牆 anchor 主要在更深度的 KP 思考筆記、教學系列 + 月度備忘錄 substack 大致免費）。

#### 數字

- 9 URL fetch 完成
- 6 重複 / 3 新建 = ~67% wiki 已收錄率（教學系列前 23 篇 + 備忘錄 6 篇 anchor 完整、追蹤 substack 從 #24 開始補延伸閱讀）
- 3 raw + 3 summary 補建
- 1 index 更新（教學系列段 + 備忘錄段）
...
## 2026-06-10 #P3 — 方法論收口：投資/研究方法總綱建檔 + 知識重新消化第一輪

**觸發**：Leo「幫我建立研究方法以及投資方法、知識庫需不需要整合延伸、有哪些地方有缺漏、知識能不能重新消化」。

### 操作

1. **新 concept ×3（方法論總綱層）**：
   - [[投資四元問題框架（ABCD）]] ⭐⭐——本對話最大的提煉（A×B×C×D、任一失敗不買）首次寫進 wiki（之前只存在於對話）；含全庫工具映射表 + 4 個活案例（SNPS/ASML/TXC/AAOI）+ 邊界規則 + KOL 視角分佈
   - [[研究方法 SOP（驗證型 ingest 與記分板閉環）]] ⭐⭐——八步迴路 + 兩層驗證四級標記 + 注碼跟隨研究深度（深度檔四項 gate）+ 「決策驅動不是素材驅動」成長原則
   - [[進場三階段與出場三階段（宋分）]]——從宋分 #24/#25 summary 抽出概念化（帳本調倉規則引用多次、一直缺檔）
2. **index.md**：方法論區頂部新增「方法論總綱」段

### 重新消化（本輪完成）

- ABCD 框架：對話 → wiki 永久資產（含 77+ concept 的四軸分類映射）
- 宋分 #24/#25：summary → 可引用 concept
- 三大教訓（SNPS／FPS-AAOI 誤判／subagent 發散）→ SOP 鐵則條文化

### 缺漏清單（#P3 盤點、按優先序）

**P0 持倉驅動（注碼跟隨研究深度 gate 不符）**：
- [[國巨]] 深度檔（22.4% 注碼 vs 淺研究：無五軸深度校準、無一手 anchor、trigger 待定）⭐ 最急
- [[欣興]] 五軸 + 出場規則（11.3% 注碼、+60% 已到出場三階段第一訊號區）
- [[Micron]] PB 分位檢查（記憶體主題穿透後 12.0%）

**P1 方法缺件（ABCD 框架點名）**：
- 三段估值架構（敘事建立→基本面追上→估值壓縮）concept
- B 軸客觀指標（EPS revision／sellside coverage／ETF 納入）——codex 指名兩次
- 檢測賣水人 pattern 概念化（TGV AXI + CPO 測試 = 已兩例、找第三例驗證跨賽道通用性）

**P2 名單缺口**：聯發科（00988A top10 唯一缺檔）、Advantest（ATE 對照組）、穎崴 6515／旺矽 6223（CPO 測試介面 watchlist）、Himax／Astera Labs／Semtech／Hammond（P1 報告點名未建）

**P3 系統運轉**：每週檢視首跑（含記分板基準價補齊 + MU PB 分位）、C 軸季度刷新機制、/weekly-review skill 化

### Surprising

1. **ABCD 框架從未進過 wiki**——全庫最重要的方法提煉只存在於對話 transcript、差點隨 compaction 蒸發；「重新消化」的最大標的是對話本身、不是 raw 素材
2. **帳本規則引用了一個不存在的檔**——調倉優先序寫「宋分出場三階段」時該 concept 還沒建檔（只有 summary）；決策層跑得比知識層快 = 決策驅動成長原則的實證

## 2026-06-10 #P4 — P0 持倉深度補齊：國巨/欣興 五軸+監控表 + MU PB 分位檢查

**觸發**：Leo「開始」（執行 #P3 缺漏清單 P0 層）。

### 操作

1. [[國巨]] **五軸 22/25 補齊**（路 5／站 4／耗 5／IP 3／客 5、confidence: high）+ 監控指標表（接單出貨比 1.3 跌破 1.0／高階報價／嵌入式擠壓／K 型防線 falsification）+ Q1 mix 更新（磁性 25.2／鉭 24.3／電阻 19.0／MLCC 13.5／感測 13.0）+ 法人 2026 指引（營收 1,587 億／EPS 16.1／毛利率挑戰 38.6%）+ 液冷商機軸
2. [[欣興]] **五軸 19/25 補齊**（路 4 玻璃自轉對沖／站 4／耗 4／IP 3／客 4）+ 監控指標表（玻璃線里程碑=falsification 候選）+ CapEx 254→**340 億**上修 + AI 佔比目標 60%+ + 外資 1,225／EPS 11.62／24.03 + **出場三階段第一訊號區判定（三訊號全亮）**
3. [[Micron]] PB 分位檢查：**P/B 6.55-13.95（依來源）vs 10 年中位 1.93（+623%）**——PB 估值法規則觸發；反方論點（HBM iPhone moment／基礎建設化 = PB 框架失效）並列註記
4. 賣水人 master：#P4 段（國巨 22／欣興 19）
5. private 帳本：兩檔 trigger/falsification 填入、MU 升彈藥④、國巨規則層更新（深度檔達標、錯配緩解）；記分板 +3 call（#9 國巨建分／#10 欣興訊號區判定／#11 MU 框架對決）

### Surprising（含一筆認錯）

1. **#P3 的「國巨/欣興淺研究」判斷是錯的**——兩檔 2026-06-05 就有完整三段式＋Re-rate 三角形＋法說數據、我沒重讀檔案就下了「淺」的結論。真正缺的只有五軸＋監控表兩件。教訓進 SOP 候選：**深度判定必須開檔驗證、不憑記憶**（與 P1「read-across 必外驗」同族）
2. **MU PB 是兩個 wiki 框架的正面對決**：PB 估值法（週期警戒）vs HBM iPhone moment（框架失效論）——記分板 #11 設計成「12mo 後輸的框架在 wiki 降權」、第一次用記分板裁決概念層衝突
3. 欣興出場第一階段三訊號（顯著漲幅／共識／估值）在同一天全亮——新建的三階段 concept 立刻有了第一個實戰判定
...
## 2026-06-10 #P7 — autoresearch 策略建立 + 規則層回測 25 輪 + 方法回寫

**觸發**：Leo `/autoresearch 幫我建立策略，並回測優化，在這個過程中也一起優化知識庫的方法`＋設定問答（目標函數=絕對報酬+DD≤20% 約束、兩種回測都跑、C/D 軸+組合規則可調、25 輪、台股為主美股為輔）。

### 操作

1. **回測基建（private/backtest/）**：自建引擎（Yahoo v8 chart API 直連、yfinance 0.2.40 請求層壞掉繞過）、19 檔宇宙 15 年日線、point-in-time（訊號 t-1）、train 2014-2022（四場危機）／test 2023-2026 **封存**、成本入帳
2. **25 輪 autoresearch**：基線（wiki 文件值直譯）score 0.0042（曝險僅 10%、規則直譯=空手九年）→ 冠軍 iter18 `gross_cap=30%＋無凍結＋宋分帶原值` score **0.0657**（train CAGR 7.3%／DD 20.4%／Sharpe 0.76）；成本×1.5 穩健；test 開封一次：CAGR 24.3%／DD 12.5%／Sharpe 2.02（危機段大勝 0050、純牛市段落後=DD 約束的代價）
3. **新 concept**：[[規則層回測發現（V 型定理與曝險前緣）]] ⭐⭐——八條定理（HWM 死鎖／雙錨分離／V 型定理六連敗／分散不防單因子／結構式>反應式／訊號純度>頻率／宋分帶通過校準／拋物線頂停損失效）
4. **[[進場三階段與出場三階段（宋分）]] #P7 校準段**：帶值驗證＋拋物線條款＋階段二前置條件缺陷修正
5. **private/策略.md v1.0**：目標函數＋台股為主美股為輔＋ABCD＋D 軸校準版＋regime 開關（基本面慢訊號、禁價格機械式）＋新資金節奏提案＋策略級 falsification；⚠️ 張力註記待 Leo 裁決（宣告 DD≤20% ↔ 真實持倉 95% invested）
6. **復盤**：國巨 2018（s3 -51% 才觸發 vs 全程 -77%）／MU·欣興·TSMC·0050 2022（-13~-28% 觸發）——拋物線條款的實證
7. 帳本回撤程序加解凍條款＋記分板 #12（規則冠軍 forward paper-track）

### Surprising

1. **wiki 文件值直譯=空手九年**——規則寫在紙上和跑在資料上是兩回事、曝險 10% 的「策略」誰都沒發現、回測第一天就抓到
2. **V 型定理（最大產出）**：六種反應式降險機制全敗——「跌了才賣」在 V 型市場結構性雙輸；風險控制的正解=平時就少拿（結構式上限）
3. **宋分 +50% 減碼帶被 9 年資料蓋章**——KOL 提煉值通過量化校準首例（KOL reliability 體系的新證據型態）
4. **負結果是結果**：19/25 輪 discard、八條定理全部來自失敗迭代——autoresearch 的 keep/discard 紀律正是價值所在
5. **目標函數與真實持倉的矛盾**是整個練習最大的策略級發現——回測沒有回答「該選哪個」、它只是讓矛盾無法再隱形

### 待辦

- Leo 裁決策略張力（DD≤20% vs 95% invested、二選一）
- 年度回測重跑機制（新資料進 train）入每週檢視 runbook 年度項
- C 軸（PB/PE 帶）量化迴圈（待歷史基本面資料源）

## 2026-06-10 #P7b — 策略張力裁決：(b) DD 容忍上修至 30%、冠軍重校準

- Leo 裁決 (b) → 目標函數修訂為「絕對報酬最大化、MaxDD ≤ 30%」
- 重校準（train 選參、test 第二次開封僅此一次）：**(b)-冠軍＝無總曝險上限、規則自然節流**（train CAGR 13.1%／DD 29.0%／自然曝險 ~66%；test CAGR 68.2%／DD 28.8%、兩段皆勝 0050）
- **定理 9 新增**：中間曝險帶（cap 55-60%）被支配——「半套防禦最糟」、防禦要嘛足額（cap 50%、餘裕 7.6pp）要嘛交給規則、不停中間
- 私有層連動：策略.md 第 1/5 節改版、帳本三線制（-10% 警示／-20% 重跑 ABCD／**-30% 預算耗盡線=只減不加**）、記分板 #12 重定基準（未起算即替換、留紀錄）
- 殘餘差距誠實註記：真實持倉 ~95% vs 規則自然曝險 ~66%——差距=出場三階段執行力、每週檢視追蹤

## 2026-06-10 #P8 — 籌碼層建立 + TXC 偏離復盤首跑（「誰在賣」有答案了）

**觸發**：Leo「某些時候股票表現跟我們預期相差甚遠、會不會少看了故事／籌碼」→「開始、傾向台股」。

### 操作

1. **新 concept**：[[籌碼面（B 軸的資金流層）]]——B 軸第二層（量表量敘事、籌碼量真錢）、台股五件套（T86／MI_MARGN／TDCC／MI_QFIIS 全公開 API）、判讀規則接 ABCD、Gate 紀律（三場景調用、防定理 6 噪音）
2. **[[B 軸客觀指標（市場認知量表）]] 升級七指標**：加⑦籌碼結構＋快查清單籌碼行；TXC 案例列修正（原判 1-2 ❌ → 2-3）
3. **chips.py 抓取器（private/chips/）**：5 檔上市持倉 2026-04-07~06-10 全樣本（三大法人＋融資券日資料、集保大戶週累積、外資持股比率）；TDCC 代號尾隨空白 bug 修正
4. **偏離復盤程序入 runbook**（>15pp 觸發五問）＋ **TXC 復盤首跑**（private/復盤/）
5. 策略 §4 加進場品質 gate；帳本國巨／晶技／德律行籌碼註記；記分板 C3 教訓入板

### TXC 復盤結論（去識別化方法層）

- 「誰在賣」：**外資（趨勢性、法說日單日 -5,405 張＝利多出貨窗）**；接手＝融資散戶（20 日近翻倍）＋投信
- 歸因：**B 軸誤判**（「法說剛公開＝市場不知道」、但價 20 日 +57% 早已說明 smart money 跑過一波——「一手資訊優勢的幻覺」）＋ **D 軸追高**（spike 頂未走試水位）；A 軸 thesis 完好
- 框架更新：①一手資訊優勢判定先過價格／籌碼檢查 ②進場品質 gate（20 日 >+40% 或融資翻倍 → 只准試水位）③籌碼欄建檔必填

### 其他持倉籌碼首讀（方法層摘要）

- 最大持倉（被動元件）：**外資 20 日大舉出、融資近倍增、價 +116%**＝「大手出槓桿接」教科書結構 → B 上修 3-4、出場第一訊號三項全亮＋籌碼佐證
- 載板持倉：外資 20 日買超＋融資下降＝籌碼健康（第一訊號區判定不變、執行可從容）
- 檢測持倉：外資賣＋價 -17.8%/20 日 → 出場第二訊號區候選
- 記分板統計 pattern：三筆已結失誤兩筆在 B 軸——**市場認知判定是系統當前最弱的軸**、籌碼層正是補這裡

### Surprising

1. **第一次拉籌碼就修正了兩個 B 階段判定**（TXC 1-2→2-3、國巨 →3-4）——盲區是真的、而且便宜（全公開 API）
2. **法說日＝出貨窗**這個 pattern：一手資訊到手的那天、可能正是 smart money 等的流動性窗口——資訊優勢與價格優勢是兩回事
3. 投信 vs 外資的對作（TXC、國巨皆然）：台股特有的「外資出、投信作帳接」結構、時限性要進監控

### 待辦

- TPEx 上櫃版抓取器（穎崴／旺矽 watchlist 用）
- 集保大戶趨勢需 4-6 週累積才能判讀
- 美股衛星持倉輕量籌碼（short interest／13F、偏離復盤時手動）

## 2026-06-10 #P9 — 全系統對抗審核（4 紅隊 + 1 實證）+ Tier 0/1 修復

**觸發**：Leo「想辦法幫我確認我們現在的做法是不是正確的」。四個獨立紅隊（codex 第三次／指數派／量化派／執行派、全部禁止禮貌性平衡）＋國巨 2018 實證。完整審核存 private/審核/。

### 五票 verdict

- **codex**：研究紀律真、決策層把校準/驗證/事後規則/真實重倉混在一起
- **指數派**：自我覺察最高的過度工程；10 筆結案前嚴格劣於 0050 定期定額
- **量化派**：流程紀律一流、統計證據三流——hedge 住註腳、「定理/confidence: high」住標題
- **執行派**：130 條規則沒有一條能自己扣板機；「不執行」不存在於任何記錄類別
- **實證 ✅**：出貨型背離在國巨 2018 峰值前 3 個月亮燈、連續 28 天（n=2、FP 未測、派發訊號非摸頭訊號）

### Tier 0 硬 bug 修復（本日完成）

帳本高 beta 綠燈矛盾（≈0% → 33.7% 🔴）／-30% 耗盡線補雙錨＋90 日解除／ABCD 總綱 TXC「四項 PASS」更新為動態重判警語／REPORT.md 加 test 二次開封降級聲明（全部結果降為描述性、不可引用為證據）／B 軸量表「通過回測」→「事後對齊」／裁判統一（策略級主判=0050）

### Tier 1 修辭重定價（本日完成）

- [[規則層回測發現（V 型定理與曝險前緣）]]：**九條定理 → 九條假說＋逐條證據分級**（1/2=B、3=C- 倖存警語、7=D、8=B-、9=C-）、confidence → low；「宋分帶通過校準」撤回（差異 0.0033 在噪音帶 ±0.003 內、改「未被否證」）
- [[投資四元問題框架（ABCD）]]：乘法 gate 加非獨立性註記（診斷框架＋致命失敗清單、非獨立機率乘法）、confidence → medium
- [[研究方法 SOP（驗證型 ingest 與記分板閉環）]]：新鐵則——「未滿 10 筆」紀律擴張到所有框架結論；新規則上線預設 flag-only、過預註冊檢驗（n≥5＋假陽性率）才升 gate；「定理/驗證/通過」為 A-B 級專用詞
- [[籌碼面（B 軸的資金流層）]]：補國巨 2018 實證＋定位明確化（紅旗→觸發重判、不自動賣）
- 私有層：待決表上線（訊號即 ticket、計齡、沉默不再無聲蒸發）；策略 V 型「明文禁止」→「預設禁止＋價格訊號只觸發重判」

### 待 Leo 批准（Tier 2 治理三件）

① ticket 協議 v2（小額再平衡預設執行＝週末條件單、否決留痕、參數真空條款、記分板執行差欄）②(b)-冠軍曝險：cap50（證據預設）vs 無上限（簽「信念非證據」）③決策日誌 15 欄→5 欄＋runbook 決策箱 ≤3 項收尾

### Tier 3 量化作業（排隊）

輸家宇宙重跑 iter9-15／bootstrap 噪音地板／walk-forward-lite／籌碼規則擴測 5-10 歷史頂部（預註冊判準）／KOL call ledger

### Surprising

1. **四個獨立紅隊的交集壓倒性集中在同一處**：訊號→訂單真空——不是知識問題、是治理問題
2. **籌碼層是唯一同時被攻擊（n=1 自我授證）又被實證救回（2018 retro ✅）的組件**——「先標 flag、再驗證、後升格」的 SOP 新鐵則就是這條經驗的條文化
3. 審核本身用了系統自己的方法（對抗驗證、預註冊、負結果留檔）——**系統的研究方法通過了「用它審它自己」的遞迴測試、敗的是執行層**

## 2026-06-10 #P9b — 治理協議 v2 批准生效（Leo「都批」）

- **協議 v2 入策略 §9**：訊號即 ticket／L2（≤5%）預設執行＝週末條件單、否決留痕／L3 逾 2 週凍結該檔／參數真空條款／裁決動詞封閉集（執行·否決·附條件延後）／記分板執行帳（執行差＝拖延的台幣成本）＋override 列（半年後用命中率數據決定預設權歸屬）／規則預算制 one-in-one-out
- **曝險簽核**：cap50＝證據預設生效（無上限未獲「信念非證據」簽核、降為研究假說）；記分板 #13 cap50 paper-track 與 #12 無上限**雙軌 12 個月、forward 數據裁決**——歷史（test 已污染）不再有投票權
- **減負**：決策日誌 15 欄→5 欄（語音可）；weekly brief 以決策箱（≤3 項）收尾；偏離復盤觸發修正（刪未定義的主題基準、改月偏離 10pp＋紅旗或 Leo 痛感）；regime 訊號 3 刪（無資料源）；新資金節奏簡化為一條
- **協議首批 ticket**：T-001／T-002 轉 L2 預設執行（掛單規格＋T+3 預設執行日入待決表）、T-003 真空條款計時中（06-24 auto 20%）
- 系統狀態：四紅隊審核 → Tier 0/1 修復 → 治理 Tier 2 批准——**#P9 審核迴圈完整閉環**；Tier 3 量化作業排隊（輸家宇宙／bootstrap／walk-forward／籌碼擴測／KOL ledger）

## 2026-06-10 #P10 — 知識庫對抗審核（5 路）+ 硬錯誤修復 23 處 + TXC C 軸翻案

**觸發**：Leo「讓紅隊審核知識庫以及構建知識庫的方式」。五路：資訊架構／認識論／事實抽查／codex（第四次）／檢索盲測。完整報告存 private/審核/。

### 五票要點

- **架構**：分數無單一真值（改 1 動 13、傳播率 40%）、index＝50K token 過期快取、summaries 孤兒 76%＋44 檔 `#` 檔名不可連結
- **認識論**：raw 71% 來自兩位多頭 KOL、空頭來源＝0、confidence 通膨（high 125/low 1）、93% check_after 同落 12 月、AI 前提崩潰殘值 ~8-9%
- **事實抽查（46 條 claim）**：全庫硬錯率 15%——**手工檔 41% vs agent＋驗證流程檔 0%**：問題不是不會驗證、是允許不驗證（「區間估」後門）
- **codex**：9 條構建規範 7 條高落差、知識點 gate 零拒收、評分 72.4% 擠在錨定帶、wikilink 是無型別連結海
- **盲測 7/10**：半小時可重建任何 thesis、但單頁快照不可信任、被取代框架不自知

### 重大翻案（已修、含私有層連動）

**TXC 財務快照整段幻覺**：股價「50-70 區間估」實為 214、「Forward PE 12-18x 估值乾淨」實為 ~33x（進場 235.7＝**~36x**）——TXC 偏離復盤 C 軸由「非主因」翻案為「主因三」（歸因改 B+C+D 三軸共因＋驗證紀律）、ABCD 案例表／帳本／SiTime 對照同步修正、記分板 C4 入板（統計：已結 4、命中 0）。其餘硬錯同修：國巨 Shibaura 44 億→7.3 億美元（差 6 倍）／EPS 15.7→11.51／股數分割後 ~20.5 億／鉭電容 >50%→~35-40%；SiTime 市值差 9 倍；Kyocera NYSE 已下市；NDK 營收；summary 第 7 層→第 4 層。

### 修復方案（Tier 1 結構／Tier 2 認識論／Tier 3 方法政策、詳 private/審核/）待 Leo 一聲「開始」。

### Surprising

1. **agent 建檔比手工乾淨（0% vs 41% 硬錯）**——顛覆直覺；錯誤率與「是否跑驗證流程」相關、與誰建檔無關；「區間估」是制度化的幻覺後門
...
## 2026-06-10 #P12 — 日頻 L1 觸發掃描器上線（「直接給買賣決定」Phase 4 地基）

**觸發**：Leo 問「變成直接給買賣決定的工具還缺什麼」→ 七缺口盤點 → 「開始」蓋第四項（數據管線日頻化）。

- **架構**（private/scan/、細節不入公開層）：portfolio 機器鏡像＋tickets.tsv 票帳＋daily_scan.py——日頻掃 L1（價格訊號→候選票、三線制 NAV、regime 線、票齡）、輸出日掃 brief
- **設計紀律**：①只產票不產單 ②籌碼維持週頻（假說 6 訊號純度>頻率、不日盯）③資料安全網——單日>30%／報價過期／**台股 Yahoo×TWSE 雙源核對不符>2% → DATA SUSPECT 不出票**；單日新票>5 張熔斷 ④L3 永遠人裁
- **首跑即見血**：雙源核對第一次運轉就解掉一個 89 點的價格矛盾（訊號價=盤中高點 vs 收盤、當日震幅 11%）——並產出執行帳第一筆延遲成本計量；候選票機制正常出票（拋物線警戒一張、自動去重既有票）
- **Surprising**：審核時說「自動化會把資料錯誤放大成訂單」是理論——上線第一天就遇到需要雙源仲裁的真實案例；安全網不是過度工程、是第一天就用上的東西

## 2026-06-10 #P13 — 「直接給買賣決定」缺口補完（方法層紀錄）

sizing 公式（單筆風險預算制：1% NAV ÷ 進場-證偽距離、三重 cap、無證偽價不出票）／訂單規格翻譯表（票→單：分批/限價帶/有效期/含稅費）／買票五腿 AND gate（L2 週頻維護腿＋L1 自動 regime 腿）／規則編譯（仲裁優先序 0-7、三層自動化邊界、同標的出場票壓買票鐵則）／影子模式（每張票 T+1 影子成交、影子組合 vs 實際 vs 0050 三軌對帳）。掃描器 v2：adjclose 修除權息假訊號類、冪等驗證通過。私有層實作、公開層僅方法。下一步：全系統紅隊（#P14）。
