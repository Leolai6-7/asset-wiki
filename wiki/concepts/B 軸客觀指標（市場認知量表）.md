...
| [[TXC]] | 法說 MEMO 2026-06-09 剛公開、sub-30fs thesis 無系統性 sellside 覆蓋 | 1-2 | 階段 1-2 ✅ |
...
四案例量表判定與既有敘事判定一致＝量表通過回測；之後新 entity 改成**量表先判、敘事後驗**（順序反轉、消除直覺先行）。
...
4. 判定：階段 [N 或 N-M]；背離訊號：B/B vs 股價[同步/背離]
```
